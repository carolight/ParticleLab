...
class ParticleLab: MTKView
{
  ...
  init(width: UInt, height: UInt, numParticles: ParticleCount, hiDPI: Bool)
  {
    ...
    let scale = UInt(UIScreen.main.scale)
    let frameWidth = hiDPI ? width / scale : width
    let frameHeight = hiDPI ? height / scale : height
  ...
  private func setUpMetal()
  {
    ...
    imageWidthFloatBuffer =  device.makeBuffer(bytes: &imageWidthFloat, length: MemoryLayout<Float>.stride, options: [])

    imageHeightFloatBuffer = device.makeBuffer(bytes: &imageHeightFloat, length: MemoryLayout<Float>.stride, options: [])
  }
  ...
  override func draw(_ rect: CGRect) {
    ...
    guard let particlesMemory = particlesMemory else { return }
    ...
    let colorBuffer = device.makeBuffer(bytes: &particleColor, length: MemoryLayout<ParticleColor>.stride, options: [])
    ...
    let dragFactorBuffer = device.makeBuffer(bytes: &dragFactor, length: MemoryLayout<Float>.stride, options: [])
    commandEncoder.setBuffer(dragFactorBuffer, offset: 0, index: 6)

    let respawnOutOfBoundsParticlesBuffer = device.makeBuffer(bytes: &respawnOutOfBoundsParticles, length: MemoryLayout<Bool>.stride, options: [])
    ...
    if !clearOnStep
    {
      let inPlaceTexture = UnsafeMutablePointer<MTLTexture>.init(bitPattern: 1)!
